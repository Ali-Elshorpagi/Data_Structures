...
{
  std::queue<std::pair<Binary_Search_Tree<type> *, std::pair<int, int>>>
      nodes_queue;
  data = level_order[0];
...
Binary_Search_Tree<type>::Binary_Search_Tree(
    const Binary_Search_Tree<type> &other)
    : data(other.data)
{
  if (other.left)
    left = new Binary_Search_Tree<type>(*other.left);
  if (other.right)
    right = new Binary_Search_Tree<type>(*other.right);
}
