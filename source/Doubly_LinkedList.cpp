...
template <class type>
Doubly_LinkedList<type>::Doubly_LinkedList(const Doubly_LinkedList<type> &another)
{
  for (D_Node<type> *cur(another.head); cur; cur = cur->next)
    insert_end(cur->data);
}

template <class type>
Doubly_LinkedList<type> &
Doubly_LinkedList<type>::operator=(const Doubly_LinkedList<type> &another)
{
  if (this != &another)
  {
    while (head)
