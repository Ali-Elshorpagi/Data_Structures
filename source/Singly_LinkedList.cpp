...
template <class type>
Singly_LinkedList<type>::Singly_LinkedList(const Singly_LinkedList<type> &another)
{
  for (S_Node<type> *cur(another.head); cur; cur = cur->next)
    insert_end(cur->data);
}

template <class type>
Singly_LinkedList<type> &
...
void Singly_LinkedList<type>::insert_sorted(type val)
{
  ...
  {
    for (S_Node<type> *cur(head), *prv(nullptr); cur;
...
void Singly_LinkedList<type>::delete_node_with_key(type val)
{
  ...
  {
    for (S_Node<type> *cur(head), *prev(nullptr); cur;
...
template <class type>
S_Node<type> *
Singly_LinkedList<type>::merge_two_lists_iterative(S_Node<type> &head1, S_Node<type> &head2)
{
...
template <class type>
S_Node<type> *
Singly_LinkedList<type>::merge_two_lists_recursive(S_Node<type> &head1, S_Node<type> &head2)
{
  if (!head1)
    return head2;
  if (!head2)
